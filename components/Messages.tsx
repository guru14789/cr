...
export const Messages: React.FC = () => {
  ...
  // Mobile View State: false = list, true = chat
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-9.5rem)] animate-in fade-in slide-in-from-bottom-4 duration-500 pb-2">
      
      {/* Page Header */}
      <div className="flex justify-between items-center mb-6 shrink-0">
        <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Message</h1>
        <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
            <span className="hover:text-primary cursor-pointer transition-colors">Home</span>
            <ChevronRight className="w-4 h-4" />
            <span className="font-bold text-slate-800 dark:text-white">Message</span>
        </div>
      </div>

      <div className="flex flex-1 gap-6 overflow-hidden min-h-0">
        {/* Left Sidebar - Contact List */}
        <div className={`w-full md:w-80 lg:w-96 bg-white dark:bg-slate-800 rounded-2xl shadow-soft border border-slate-100 dark:border-slate-800 flex flex-col overflow-hidden shrink-0 ${showChatOnMobile ? 'hidden md:flex' : 'flex'}`}>
           <div className="p-5 border-b border-slate-100 dark:border-slate-800">
              ...
              <div className="relative">
                 ...
                 <input 
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary-100 dark:text-white transition-all"
                 />
              </div>
           </div>
        ...
        {/* Main Chat Area */}
        <div className={`flex-1 bg-white dark:bg-slate-800 rounded-2xl shadow-soft border border-slate-100 dark:border-slate-800 flex-col overflow-hidden min-w-0 ${showChatOnMobile ? 'flex' : 'hidden md:flex'}`}>
           {/* Chat Header */}
           ...
           <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/30 dark:bg-slate-900/30">
              ...
              {messages.map((msg) => (
                 <div key={msg.id} className={`flex ${msg.isMe ? 'justify-end' : 'justify-start'} group`}>
                    ...
                    <div className={`max-w-[85%] md:max-w-[70%] ${msg.isMe ? 'items-end' : 'items-start'} flex flex-col`}>
           ...
           <div className="p-4 bg-white dark:bg-slate-800 border-t border-slate-100 dark:border-slate-700">
              <div className="flex items-end gap-3">
                 <div className="hidden sm:flex gap-1 mb-2.5 text-slate-400">
                 ...
                 <button className="sm:hidden mb-2.5 p-2 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg">
                 ...
                 <div className="flex-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl flex items-center p-2 focus-within:ring-2 focus-within:ring-primary-100 focus-within:border-primary-300 transition-all">
                    ...
                    />
                    <button className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors hidden sm:block">
